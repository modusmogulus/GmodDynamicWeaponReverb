...
local function changeBlacklist(action)
	...
	if not IsValid(weapon) then
		print("Weapon invalid. Blacklist not affected!")
...
local function traceableToSky(pos, offset)
	local tr = util.TraceLine({
		start = pos + offset,
		endpos = pos + Vector(offset.x, offset.y, 100000000),
		mask = MASK_GLOBAL
	})

	-- doing this because sometimes the trace can go oob and even rarely there are cases where i cant see if it spawned oob
	local temp = util.TraceLine({
		start = tr.StartPos,
		endpos = pos,
		mask = MASK_GLOBAL
	})
...
local function playReverb(src, ammotype, isSuppressed, weapon)
	...
	if GetConVar("cl_dwr_disable_indoors_reverb"):GetBool() == true and positionState == "indoors" then return end
	if GetConVar("cl_dwr_disable_outdoors_reverb"):GetBool() == true and positionState == "outdoors" then return end
	...
	if GetConVar("cl_dwr_disable_soundspeed"):GetInt() == 1 then
		soundspeed = 0
	end
