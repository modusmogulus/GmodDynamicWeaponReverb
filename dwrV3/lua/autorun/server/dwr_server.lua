...
local function getSuppressed(weapon, weaponClass)
    if string.StartWith(weaponClass, "arccw_") and weapon:GetBuff_Override("Silencer") then
        return true
    elseif string.StartWith(weaponClass, "tfa_") and weapon:GetSilenced() then
        return true
    ...
    elseif string.StartWith(weaponClass, "cw_") then
        if weapon.ActiveAttachments ~= nil then
            for k, v in pairs(weapon.ActiveAttachments) do
                ...
                if att.isSuppressor then return true end
            end
        end
    end
...
local function networkGunshotEvent(data)
    ...
    if networkGunshotsConvar:GetBool() then
...
function arc9_dwr_detour(args)
    ...
    if attacker.dwr_shotThisTick == nil then
        attacker.dwr_shotThisTick = false
    end
...
hook.Add("EntityFireBullets", "dwr_EntityFireBullets", function(attacker, data)
    ...
    -- should solve all of the issues caused by external bullet sources (such as the turret mod)
    if not weaponIsWeird and weapon ~= NULL and entity.GetShootPos ~= nil then
        local weaponClass = weapon:GetClass()
        ...
        if string.StartWith(weaponClass, "arccw_") then
            if data.Distance == 20000 then return end -- grenade launchers in arccw
            if GetConVar("arccw_bullet_enable"):GetInt() == 1 and data.Spread == Vector(0, 0, 0) then return end -- bullet physics in arcw
        end

        if string.StartWith(weaponClass, "arc9_") then
            if GetConVar("arc9_bullet_physics"):GetInt() == 1 and data.Spread == Vector(0, 0, 0) then return end -- bullet physics in arc9
        end
